...
% Initial estimates
theta_1(:,1) = 3;       % Initial estimate of k
theta_2(:,1) = [10; 0.9];   % Initial estimate of m and beta
...
for n = 1:N-1
    ...
    % m_dot_hat
    if (theta_2(1,n) > m_min) || (theta_2(1,n) == m_min && theta_2_dot(1) >= 0) 
        theta_2_dot(1) = gamma_2(1,1) * epsilon_2 * phi_2(1);
    else 
        theta_2_dot(1) = 0;
    end

    % beta_dot_hat 
    if((beta_max > theta_2(2,n)) && (theta_2(2,n) > beta_min)) || (theta_2(2,n) == beta_min && theta_2_dot(2) <= 0) || (theta_2(2,n) == beta_max && theta_2_dot(2) <= 0) 
        theta_2_dot(2) = gamma_2(2,2) * epsilon_2 * phi_2(2);
    else
        theta_2_dot(2) = 0;
    end
    ...
    % k_dot_hat
    if (theta_1(n) > k_min)    || (theta_1(n) == k_min && theta_1_dot >= 0)
