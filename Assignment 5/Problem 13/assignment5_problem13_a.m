...
m                       = 15;
k                       = 2;
% 1/Lambda [u] = s^2 / Lambda [y_2] * m + s/Lambda [y_2] * beta
...
u           = 5 * sin(2 * t) + 10.5;
beta_1_ls   = 1;    % Forgetting factor
beta_2_ls   = 1;    % Forgetting factor
...
theta_2(:,1) = [14; 0.3];   % Initial estimate of m and beta
Q_1_0 = 20;
Q_2_0 = diag([66 2]);
P_1 = Q_1_0\eye(1); 
P_2 = Q_2_0\eye(2);
...
for n = 1:N-1
    ...
    % Update law
    theta_1_dot       = P_1 * epsilon_1 * phi_1;                    
    P_1_dot           = beta_1_ls * P_1 - P_1 * (phi_1 * phi_1') * P_1;
    
    theta_1(:, n+1)   = theta_1(:, n) + theta_1_dot*h;
    P_1                = P_1 + P_1_dot * h;
    
    theta_2_dot       = P_2 * epsilon_2 * phi_2 ;                    
    P_2_dot           = beta_2_ls * P_2 - P_2 * (phi_2 * phi_2') * P_2;
...
grid
subplot(3,1,2)
plot(t, theta_2(2,:)./theta_2(1,:)); hold on
